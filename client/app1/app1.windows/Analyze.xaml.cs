...
namespace App1
{
    ...
    /// </summary>
    public sealed partial class Analyze : Page, IActionStarter
    {
        ...
        private void Timer_Tick(object sender, object e)
        {
            try{
                ...
                if (Waardes!=null && Waardes.Count >= frame - 1)
                {
                    var finalValue = Waardes[frame - 1];
                    if (finalValue < 0)
                    {
                        finalValue = 0;
                    }
                    WaardeBox.Text = "" + finalValue;  
                }
                ...
                if (success)
                {
                    ...
                    FramerateService.Get(delegate(bool succes, string fps)
                    {
                        try
                        {
                            ...
                            Fps = fpsd;
                            Timer.Interval = TimeSpan.FromMilliseconds((double)1 / fpsd * 1000);
                        }
                        catch (Exception ex) { }
                    }, selectedItem.Naam);
        ...
        private void Button_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                NetworkService.Run(delegate(bool success, IList<double> waardes) {
                    Waardes = waardes;
                    Dispatcher.RunAsync(Windows.UI.Core.CoreDispatcherPriority.Normal, () =>
                           {
                               MediaPlayer.Play();
                           });
                }, GekozenTrick, GekozenVideo);
            }
            else
            {
